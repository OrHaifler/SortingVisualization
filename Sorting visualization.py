...
def draw(draw_info, algo_name, ascending):
	...
	sorting = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort | S - Selection Sort", 1, draw_info.RED)
	...
	if ascending:
		for i in range(len(lst)):
			...
			if i!=id:
				...
				draw_list(draw_info, {i: draw_info.GREEN, id: draw_info.RED}, True)
				yield True
	else:
		for i in range(len(lst)):
			id = i
			for j in range(i+1, len(lst)):
				if lst[j] > lst[id]:
					id = j
				lst[i], lst[id] = lst[id], lst[i]
				draw_list(draw_info, {id: draw_info.GREEN, i: draw_info.RED}, True)
				yield True
